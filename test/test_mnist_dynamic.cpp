...
void RunMnistModel(const std::string &model_path,
  ...
  auto intput_tensor = engine.GetInputTensors().at("input");
  ...
  //   auto result = img_utils::Softmax(output_tensor_.p,
  //   output_tensor_.mem_size,
  //                                    output_tensor_.data_type);
  //   ASSERT_TRUE(result.size() == 10) << "Invalid output size: " <<
  //   result.size();
