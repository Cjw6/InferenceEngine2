...
public:
   ...
   * @param input 是否为输入张量
   */
  TensorInfo(const std::string name, const nvinfer1::Dims &shape,
             const nvinfer1::DataType dtype, const bool input,
   ...
   */
  void update() {
    bytes_ = std::accumulate(shape.d, shape.d + shape.nbDims, 1,
                             std::multiplies<int>()) *
             dtype_to_bytes(dtype_);
