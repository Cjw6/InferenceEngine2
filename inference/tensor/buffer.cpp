...
#include <cpptoolkit_cuda/cuda_check.h>
...
void DeviceBuffer::allocate(size_t size) {
  if (size > size_) {
    free();
    CTK_CUDA_CHECK(cudaMalloc(&device_, size)); // < 分配设备内存
    size_ = size;
  }
}
...
void DeviceBuffer::free() {
  if (device_)
    CTK_CUDA_CHECK(cudaFree(device_)); // < 释放设备内存
  device_ = nullptr;
  size_ = 0;
}
...
void DiscreteBuffer::allocate(size_t size) {
  if (size > size_) {
    free();
    CTK_CUDA_CHECK(cudaMallocHost(&host_, size)); // < 分配主机内存
    CTK_CUDA_CHECK(cudaMalloc(&device_, size));   // < 分配设备内存
    size_ = size;
  }
}
...
void DiscreteBuffer::free() {
  if (host_) {
    CTK_CUDA_CHECK(cudaFreeHost(host_)); // < 释放主机内存
...
void DiscreteBuffer::hostToDevice(cudaStream_t stream) {
  if (stream) {
    CTK_CUDA_CHECK(cudaMemcpyAsync(device_, host_, size_,
                                   cudaMemcpyHostToDevice,
                                   stream)); // < 异步拷贝主机到设备
  } else {
    CTK_CUDA_CHECK(cudaMemcpy(device_, host_, size_,
...
void DiscreteBuffer::deviceToHost(cudaStream_t stream) {
  if (stream) {
    CTK_CUDA_CHECK(cudaMemcpyAsync(host_, device_, size_,
                                   cudaMemcpyDeviceToHost,
                                   stream)); // < 异步拷贝设备到主机
  } else {
    CTK_CUDA_CHECK(cudaMemcpy(host_, device_, size_,
...
void UnifiedBuffer::allocate(size_t size) {
  if (size > size_) {
    free();
    CTK_CUDA_CHECK(cudaMallocManaged(&host_, size)); // < 分配统一内存
...
void UnifiedBuffer::free() {
  if (host_)
    CTK_CUDA_CHECK(cudaFree(host_)); // < 释放统一内存
  host_ = nullptr;
  device_ = nullptr;
  size_ = 0;
}
...
void MappedBuffer::allocate(size_t size) {
  if (size > size_) {
    ...
    CTK_CUDA_CHECK(
        cudaHostGetDevicePointer(&device_, host_, 0)); // < 获取设备指针
    size_ = size;
  }
}
...
void MappedBuffer::free() {
  if (host_)
    CTK_CUDA_CHECK(cudaFreeHost(host_)); // < 释放映射内存
  host_ = nullptr;
  device_ = nullptr;
  size_ = 0;
}
